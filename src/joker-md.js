const VRMD = {
	...
	'tag' : {
		...
		'tCode'	: ['<pre><code class="language-$lang">','</code></pre>\n'],//lang会被替换被具体语言,若没设置具体语言则为空白字符串,highlight => data-lang="$lang"
		'tList'	: ['<li>','<ol>','<ul>','</li>\n','</ol>\n','</ul>\n'],
		'tP'	: ['<p>','</p>'],
	...
	'md2html' : function(md,options){
		//return VirEditor.handlerLines(md.split(/[\n]/g));//这种方法会导致不同系统的文档显示出现问题
		try{
		...
		for(var i=0,last=0,len=md.length;i<len;i++,line=null){
			...
			//判断一下是否为参考式
			if(line!=null){
				if(!VRMD._matchRefer(line))lines.push(line);
				last=i+1;
			}
		}
		if(last<len){
			line = md.slice(last);
			if(!VRMD._matchRefer(line))
				lines.push(line);
		}
		return VRMD.handlerLines(lines)+VRMD.handlerFoot();//内容最后如果有脚注就输出脚注内容
	...
	'handlerLines' : function(mds,inBq){
		...
		var ignoreLine = 0; //该行是否在编译时忽视(即保持原样),大于等于1就忽略
		...
		for(var i=0;i<mds.length;i++){
			...
			if(lineLeft < 4){
				if(lineTrim.slice(0,3)=='```'){//进入代码块
					...
					// console.log(res);
					// console.log(this.tag.tCode);
					nowInCode = 1;continue;
				}

				if(lineTrim.slice(0,3)=="'''"){ //-------------------------------->
					ignoreLine = 1;continue;
				}
				...
				for(var j=i+1,done=0;j<len;j++){switch(t[j]){
					...
					case ']'://找到可链接元素的标题/文本部分结束符了
						...
						if((nextLoc = t.indexOf(toFind,tI)) != -1){//正常收尾
							...
							if(linkType=='i')//输出图片
                                r += ilt.tImg[0] +
                                     this._htmlSafer(linkTitle) +
                                     ilt.tImg[1] +
                                     this._htmlSafer(linkMore) +
                                     ilt.tImg[2] +
                                     encodeURI(linkURL) +
                                     ilt.tImg[3];
                                ...
                                r += ilt.tA[0] +
                                     ...
                                     encodeURI(linkURL) +
                                     ilt.tA[2] +
                                     this.handlerInline(linkTitle,0) +
                                     ilt.tA[3];
							done=1;i=nextLoc;
						}
						j=len;
